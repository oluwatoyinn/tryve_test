        ...
        {paymentsWithDaysLeft.map((payment) => (
          <div key={payment.id} className="px-6 py-4 flex items-center">
            ...
            <div className="flex flex-col items-end">
              <span className="text-sm font-medium text-gray-800">${payment.amount.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
              <span className={`text-xs ${payment.daysLeft <= 3 ? 'text-red-500' : 'text-gray-500'}`}>
                {payment.daysLeft > 0 ? `${payment.daysLeft} days left` : 'Due today'}
              </span>
            </div>
          </div>
        ))}
