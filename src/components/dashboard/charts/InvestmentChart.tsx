...
const InvestmentChart = () => {
  ...
  const normalizeY = (value: any) => {
    return (
      chartHeight -
      ((value - minValue + padding) / (maxValue - minValue + padding * 2)) *
        chartHeight
    );
  };
    ...
    <div className="bg-white rounded-xl shadow p-6">
      ...
      <div className="mt-4 flex justify-between text-sm text-gray-600">
        <div>
          ...
          <div>{nairaFormat.format(investmentHistory[0].amount)}</div>
        </div>
        <div className="text-center">
          ...
          <div className="text-green-600 font-medium">
            +
            {nairaFormat.format(
              investmentHistory[investmentHistory.length - 1].amount -
                investmentHistory[0].amount
            )}
          </div>
        </div>
